  ...
  async (id, { dispatch }) => {
    const { data } = await axios.get(
      id
        ...
        : `${process.env.REACT_APP_API_URL || "http://localhost:8888/"}tweets`
    );
...
export const getOneTwitt = createAsyncThunk(
  ...
  async (id) => {
    const { data } = await axios.get(
      `${process.env.REACT_APP_API_URL || "http://localhost:8888/"}tweet/${id}`
    );
    return [data.data];
  }
);
  ...
  async (payload, { dispatch }) => {
    try {
      const { data } = await axios.post(
        `${process.env.REACT_APP_API_URL || "http://localhost:8888/"}tweet`,
        payload
      );
      console.log(data);
      dispatch(addTweet(data.data));
